...
```shell
java -jar target/camunda-spring-boot-amqp-ms-cloud-example-0.0.1-SNAPSHOT.jar
```
...
---
applications:
  - name: camunda-spring-boot-amqp-ms-cloud-example
    memory: 1G
    instances: 1
    random-route: false
...
```
mvn clean install && cf push -p target/camunda-spring-boot-amqp-ms-cloud-example-0.0.1-SNAPSHOT.jar
```

There it is, now you can start a process:

```shell
url -X POST -F 'orderId=123' -F 'amount=4990' http://camunda-spring-boot-amqp-ms-cloud-example.cfapps.io/order
```
